...
def _classification_rules():
    return [
        ...
        ('mask_token', 'backbone.mask_token'),
...
def _build_mapping_stats(original_state_dict, mapped_state_dict, renamed_pairs, dropped_classifier_keys):
    ...
    for key in unmatched_input_keys:
        ...
        elif any(token in lowered for token in ['mask', 'patch', 'token', 'pos']):
            unmatched_categories['mask_related'].append(key)
...
def map_pointmae_cls_state_dict(state_dict, pretrained_source='official', reset_classifier=False):
...
def load_pointmae_cls_pretrained(model, config):
    pointmae_cfg = config.model.pointmae
    if not pointmae_cfg.pretrained:
        return

    checkpoint_path = pathlib.Path(pointmae_cfg.pretrained_path).expanduser()
    checkpoint = torch.load(checkpoint_path, map_location='cpu')
    raw_state_dict = _extract_state_dict(checkpoint)
    mapped = map_pointmae_cls_state_dict(
        raw_state_dict,
        pretrained_source=pointmae_cfg.pretrained_source,
        reset_classifier=pointmae_cfg.reset_classifier,
    )
    ...
    missing, unexpected = model.load_state_dict(
        compatible_state_dict,
        strict=pointmae_cfg.strict_pretrained_load and not pointmae_cfg.reset_classifier and not shape_mismatches,
    )
    return {
        'task': mapped['task'],
...
def diagnose_pointmae_state_dict_mapping(state_dict, reset_classifier=False,
