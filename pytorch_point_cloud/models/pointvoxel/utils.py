...
def _segmentation_rules():
    return [
        ('stem', 'backbone.stem'),
        ('point_blocks.0', 'backbone.blocks.0.point_branch.net'),
        ('point_blocks.1', 'backbone.blocks.1.point_branch.net'),
        ('point_blocks.2', 'backbone.blocks.2.point_branch.net'),
        ('voxel_blocks.0', 'backbone.blocks.0.voxel_branch.net'),
        ('voxel_blocks.1', 'backbone.blocks.1.voxel_branch.net'),
        ('voxel_blocks.2', 'backbone.blocks.2.voxel_branch.net'),
        ('fusion_blocks.0', 'backbone.blocks.0.fusion'),
        ('fusion_blocks.1', 'backbone.blocks.1.fusion'),
        ('fusion_blocks.2', 'backbone.blocks.2.fusion'),
        ('classifier.', 'seg_head.'),
        ('head.', 'seg_head.'),
    ]
...
def _build_mapping_stats(original_state_dict, mapped_state_dict, renamed_pairs, dropped_classifier_keys):
    ...
    for key in unmatched_input_keys:
        ...
        elif any(token in lowered for token in ['point', 'stem', 'mlp']):
...
def map_pointvoxel_seg_state_dict(state_dict, pretrained_source='official', reset_classifier=False):
...
def load_pointvoxel_seg_pretrained(model, config):
    pointvoxel_cfg = config.model.pointvoxel
    if not pointvoxel_cfg.pretrained:
        return

    checkpoint_path = pathlib.Path(pointvoxel_cfg.pretrained_path).expanduser()
    checkpoint = torch.load(checkpoint_path, map_location='cpu')
    raw_state_dict = _extract_state_dict(checkpoint)
    mapped = map_pointvoxel_seg_state_dict(
        raw_state_dict,
        pretrained_source=pointvoxel_cfg.pretrained_source,
        reset_classifier=pointvoxel_cfg.reset_classifier,
    )
    ...
    missing, unexpected = model.load_state_dict(
        compatible_state_dict,
        strict=pointvoxel_cfg.strict_pretrained_load and not pointvoxel_cfg.reset_classifier and not shape_mismatches,
    )
    return {
        'task': mapped['task'],
...
def diagnose_pointvoxel_state_dict_mapping(state_dict, reset_classifier=False,
