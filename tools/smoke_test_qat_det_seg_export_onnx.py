...
def main():
    with tempfile.TemporaryDirectory(prefix='qat_det_seg_export_onnx_') as tmp_dir:
        tmp = pathlib.Path(tmp_dir)
        output_dir = tmp / 'qat_det_seg_run'
        checkpoint_dir = output_dir / 'artifacts'
        checkpoint_path = checkpoint_dir / 'checkpoint_00001.pth'
        onnx_path = tmp / 'quantized_det_seg.onnx'
        float_onnx_path = tmp / 'quantized_det_seg.float.onnx'

        train_command = [
            PYTHON,
            'scripts/det_seg/train.py',
            '--config',
            'configs/det_seg/shared_backbone_tiny.yaml',
            'device', 'cpu',
        ...
        if train_result.returncode != 0:
            print('FAIL | det-seg quantized onnx smoke | training failed')
            print(train_result.stdout)
            sys.exit(1)

        if not checkpoint_path.exists():
            print(f'FAIL | det-seg quantized onnx smoke | checkpoint not found: {checkpoint_path}')
            sys.exit(1)
        ...
        if export_result.returncode != 0:
            print('FAIL | det-seg quantized onnx smoke | export failed')
            print(export_result.stdout)
            sys.exit(1)

        if not onnx_path.exists():
            print(f'FAIL | det-seg quantized onnx smoke | quantized onnx file not found: {onnx_path}')
            sys.exit(1)
        if not float_onnx_path.exists():
            print(f'FAIL | det-seg quantized onnx smoke | float onnx file not found: {float_onnx_path}')
            sys.exit(1)
        ...
        verify_command = [
            ...
            (
                ...
                "print('OK | det-seg quantized onnx export | nodes=', len(model.graph.node), 'has_quant_ops=', any(op in node_types for op in ('MatMulInteger','DynamicQuantizeLinear','QuantizeLinear','DequantizeLinear')), 'det_shape=', tuple(outputs[0].shape), 'seg_shape=', tuple(outputs[1].shape))"
            )
        ]
        verify_result = run_command(verify_command)
        if verify_result.returncode != 0:
            print('FAIL | det-seg quantized onnx smoke | onnx verification failed')
