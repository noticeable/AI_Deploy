...
def main():
    with tempfile.TemporaryDirectory(prefix='distill_detection_') as tmp_dir:
        tmp = pathlib.Path(tmp_dir)
        teacher_checkpoint_path = tmp / 'teacher_checkpoint.pth'
        student_checkpoint_path = tmp / 'student_checkpoint.pth'
        train_config_path = tmp / 'dummy_distill_train.yaml'
        ...
        if result.returncode != 0:
            print('SKIP | detection distill smoke | environment not ready to create dummy checkpoints')
            print(result.stdout)
            return
...
"""
        ...
        create_dataset_command = [
            ...
            (
                ...
                "Image.new('RGB', (64, 64), color=(123, 50, 10)).save(train_image); "
                "Image.new('RGB', (64, 64), color=(123, 50, 10)).save(val_image); "
        ...
        if result.returncode != 0:
            print('SKIP | detection distill smoke | failed to create dummy dataset assets')
            print(result.stdout)
            return
        ...
        train_command = [
            ...
            'task', 'detection',
            'device', 'cpu',
            'train.checkpoint', str(student_checkpoint_path),
        ...
        if result.returncode != 0:
            print('FAIL | detection distill smoke | training command failed')
            print(result.stdout)
            sys.exit(1)

        checkpoint_files = sorted(output_dir.glob('checkpoint_*.pth'))
        if not checkpoint_files:
            print(f'FAIL | detection distill smoke | no checkpoint created in {output_dir}')
            sys.exit(1)

        print('OK | detection distill smoke | checkpoint=', checkpoint_files[-1].name)
