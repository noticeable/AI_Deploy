...
def det_seg_collate_fn(batch: List):
    ...
    for image_index, (_, target) in enumerate(batch):
        ...
        target['segmentation_mask'] = torch.as_tensor(target['segmentation_mask'], dtype=torch.long)
        target['batch_index'] = torch.full((len(target['labels']),), image_index, dtype=torch.long)
...
def create_dataloader(config, is_train):
    ...
    from pytorch_det_seg.datasets import create_dataset
